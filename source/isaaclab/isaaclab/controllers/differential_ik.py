...
def apply_customize_delta_pose(
    ...
    target_pos, target_rot = math_utils.combine_frame_transforms(
        source_pos, source_rot, delta_pose[:, 0:3], delta_quat)
    # if torch.linalg.norm(target_pos - source_pos) > 0.01:
    #     import pdb
    #     pdb.set_trace()
    #print("controller target_pos", target_pos - source_pos)
...
class DifferentialIKController:
    ...
    def set_command(self,
        ...
        if self.cfg.command_type == "position":
            ...
            if self.cfg.use_relative_mode:
                ...
                # if self.cfg.customize_delta_pose:
                #     self.ee_pos_des, self.ee_quat_des = apply_customize_delta_pose(
    ...
    def compute(self, ee_pos: torch.Tensor, ee_quat: torch.Tensor,
        ...
        # print("accept des", torch.linalg.norm(self._command[:, :3]),
        #       torch.linalg.norm(position_error))
