...
class SimulationContext(_SimulationContext):
    ...
    * adding and removing callbacks to different simulation events such as physics stepping, rendering, etc.
    ...
    def __init__(self, cfg: SimulationCfg | None = None):
        ...
        carb_settings_iface.set_bool("/physics/collisionConeCustomGeometry",
                                     False)
        carb_settings_iface.set_bool(
        ...
        self._livestream_gui = carb_settings_iface.get(
        ...
        self._offscreen_render = bool(
            carb_settings_iface.get("/isaaclab/render/offscreen"))
        # read flag for whether the default viewport should be enabled
        self._render_viewport = bool(
            carb_settings_iface.get("/isaaclab/render/active_viewport"))
        ...
        carb_settings_iface.set_bool("/rtx/translucency/enabled",
                                     self.cfg.render.enable_translucency)
        carb_settings_iface.set_bool("/rtx/reflections/enabled",
                                     self.cfg.render.enable_reflections)
        carb_settings_iface.set_bool(
            "/rtx/indirectDiffuse/enabled",
            self.cfg.render.enable_global_illumination)
        carb_settings_iface.set_bool("/rtx/transient/dlssg/enabled",
                                     self.cfg.render.enable_dlssg)
        carb_settings_iface.set_int("/rtx/post/dlss/execMode",
                                    self.cfg.render.dlss_mode)
        carb_settings_iface.set_bool("/rtx/directLighting/enabled",
                                     self.cfg.render.enable_direct_lighting)
        carb_settings_iface.set_int(
            "/rtx/directLighting/sampledLighting/samplesPerPixel",
            self.cfg.render.samples_per_pixel)
        carb_settings_iface.set_bool("/rtx/shadows/enabled",
                                     self.cfg.render.enable_shadows)
        carb_settings_iface.set_bool("/rtx/ambientOcclusion/enabled",
                                     self.cfg.render.enable_ambient_occlusion)
        # set denoiser mode
        try:
        ...
        if not builtins.ISAAC_LAUNCHED_FROM_TERMINAL:
            ...
            self._app_control_on_stop_handle = timeline_event_stream.create_subscription_to_pop_by_type(
                ...
                _app_control_on_stop_callback(*args),
                order=15,
            )
        else:
    ...
    def _app_control_on_stop_callback(self, event: carb.events.IEvent):
        ...
        try:
            profiler_tracy = carb.profiler.acquire_profiler_interface(
