...
from pxr import PhysxSchema, Usd, UsdPhysics, Sdf
...
def modify_rigid_body_properties(prim_path: str,
    ...
    if rigid_body_prim.GetAttribute("xformOp:orient").GetTypeName() == "quatf":
        current_orientation = Gf.Quatf(
            rigid_body_prim.GetAttribute("xformOp:orient").Get())
        # xform_op_rot = UsdGeom.XformOp(rigid_body_prim.GetAttribute("xformOp:orient"))
        # xform_op_rot.Set(current_orientation)
        rigid_body_prim.GetAttribute("xformOp:orient").Set(current_orientation)

        #omni.kit.commands.execute("ChangePropertyCommand", prop_path=Sdf.Path(f"{prim_path}.xformOp:orient"),value=current_orientation,prev=None,)
...
def modify_deformable_body_properties(
    ...
    # import isaacsim.core.utils.prims as prim_utils
    # mesh_path = physx_deformable_api.GetPath().pathString

    # stage_id = omni.usd.get_context().get_stage_id()

    # usdrt_stage = usdrt.Usd.Stage.Attach(stage_id)
    # boid_root = usdrt_stage.GetPrimAtPath(usdrt.Sdf.Path("/World/envs/env_0/sweep01/Mesh/Mesh"))

    # pos = boid_root.GetAttribute("xformOp:translate").Get()
    # boid_root.CreateAttribute("_worldPosition", usdrt.Sdf.ValueTypeNames.Double3, True)
    # scale = boid_root.GetAttribute("xformOp:scale").Get()

    # # boid_root.CreateAttribute("_worldScale", usdrt.Sdf.ValueTypeNames.Float3, True).Set(scale)
    # # orient = boid_root.GetAttribute("xformOp:orient").Get()

    # boid_root.CreateAttribute("_worldOrientation", usdrt.Sdf.ValueTypeNames.Quatf, True)
...
@apply_nested
def modify_particle_body_properties(prim_path: str,
                                    cfg: schemas_cfg.ParticleBodyPropertiesCfg,
                                    stage: Usd.Stage | None = None):
    ...
    # get deformable-body USD prim
    particle_body_prim = stage.GetPrimAtPath(prim_path)

    # check if the prim is valid and has the particle-cloth-body API
    if not particle_body_prim.IsValid(
    ) or not PhysxSchema.PhysxParticleClothAPI(particle_body_prim):
        return False

    # retrieve the physx deformable-body api
    physx_particle_body_api = PhysxSchema.PhysxParticleClothAPI(
        particle_body_prim)
    # retrieve the physx deformable api
    physx_particle_api = PhysxSchema.PhysxParticleAPI(physx_particle_body_api)
    cloth_auto_api = PhysxSchema.PhysxAutoParticleClothAPI.Apply(
        particle_body_prim)
    particle_api = PhysxSchema.PhysxParticleAPI(physx_particle_body_api)

    # convert to dict
    cfg = cfg.to_dict()
    # set into deformable body API

    # obtain the PhysX collision API (this is set when the particle body is added)
    physx_collision_api = PhysxSchema.PhysxCollisionAPI(particle_body_prim)

    particle_system = PhysxSchema.PhysxParticleSystem.Define(
        stage, prim_path.replace("/Mesh/Mesh", "/ParticleSystem"))

    # set into PhysX API

    for attr_name, value in cfg.items():
        if attr_name in [
                "rest_offset", "collision_offset", "Solid_Rest_Offset"
        ]:
            safe_set_attribute_on_usd_schema(particle_system,
                                             attr_name,
                                             value,
                                             camel_case=True)
        elif attr_name in ["Pressure"]:
            safe_set_attribute_on_usd_schema(physx_particle_body_api,
                                             attr_name,
                                             value,
                                             camel_case=True)

        elif attr_name in ["self_collision", "self_collision_filter"]:
            safe_set_attribute_on_usd_schema(particle_api,
                                             attr_name,
                                             value,
                                             camel_case=True)
            ...
            safe_set_attribute_on_usd_schema(cloth_auto_api,
                                             attr_name,
                                             value,
                                             camel_case=True)
