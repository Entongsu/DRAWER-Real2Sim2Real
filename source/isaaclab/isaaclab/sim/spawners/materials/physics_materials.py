...
def spawn_rigid_body_material(
    ...
    if not prim_utils.is_prim_path_valid(prim_path):
        _ = UsdShade.Material.Define(stage_utils.get_current_stage(),
                                     prim_path)
...
@clone
def spawn_particle_body_material(
        prim_path: str,
        cfg: physics_materials_cfg.ParticleBodyMaterialCfg) -> Usd.Prim:
    """Create material with deformable-body physics properties.
    ...
    physx_particle_body_material_api = PhysxSchema.PhysxPBDMaterialAPI(prim)
    if not physx_particle_body_material_api:
        physx_particle_body_material_api = PhysxSchema.PhysxPBDMaterialAPI.Apply(
            prim)
    ...
    for attr_name, value in cfg.items():
        ...
        safe_set_attribute_on_usd_schema(physx_particle_body_material_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # return the prim
