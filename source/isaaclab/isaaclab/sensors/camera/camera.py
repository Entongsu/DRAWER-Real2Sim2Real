...
from .utils import convert_camera_frame_orientation_convention, create_rotation_matrix_from_view
import warp as wp
if TYPE_CHECKING:
...
class Camera(SensorBase):
    ...
    def set_intrinsic_matrices(self,
        ...
        for i, intrinsic_matrix in zip(env_ids, matrices):
            ...
            if params["horizontal_aperture_offset"] > 1e-4 or params[
                    "vertical_aperture_offset"] > 1e-4:
                carb.log_warn(
        ...
        if len(self._data.output.sorted_keys) == 0:
            ...
            for name, annotators in self._rep_registry.items():
                ...
                for index in env_ids:
                    ...
                    # process the output
                    data, info = self._process_annotator_output(
                        name, output, index)
                    # add data to output
    ...
    def _create_buffers(self):
        ...
        self._data.quat_w_world = torch.zeros((self._view.count, 4),
                                              device=self._device)
        self._data.quat_local = torch.zeros((self._view.count, 4),
                                            device=self._device)
        self._data.quat_w = torch.zeros((self._view.count, 4),
                                        device=self._device)
        # -- intrinsic matrix
        ...
        self._data.output = TensorDict({},
                                       batch_size=self._view.count,
                                       device=self.device)
        self._data.info = [{
            name: None
            for name in self.cfg.data_types
        } for _ in range(self._view.count)]
        self.init_poses, self.init_quats = self._view.get_world_poses(
            torch.arange(self._view.count).to(self.device))
    ...
    def _update_intrinsic_matrices(self, env_ids: Sequence[int]):
        ...
        for i in env_ids:
            ...
            horiz_aperture_offset = sensor_prim.GetHorizontalApertureOffsetAttr(
            ).Get()
            vert_aperture_offset = sensor_prim.GetVerticalApertureOffsetAttr(
            ).Get()
            # get viewport parameters
            height, width = self.image_shape
    ...
    def _process_annotator_output(
            ...
            index: Any) -> tuple[torch.tensor, dict | None]:
