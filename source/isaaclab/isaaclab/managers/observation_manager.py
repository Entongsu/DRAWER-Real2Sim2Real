...
class ObservationManager(ManagerBase):
            ...
            if self._group_obs_concatenate[group_name]:
                try:
                    ...
                    self._group_obs_dim[group_name] = tuple(
                        torch.sum(torch.stack(term_dims, dim=0),
                                  dim=0).tolist())
                except RuntimeError:
                    raise RuntimeError(
    ...
    def compute_group(
            self, group_name: str) -> torch.Tensor | dict[str, torch.Tensor]:
                    ...
                    for mod_cfg in term_cfg.modifiers:
                        ...
                        if isinstance(mod_cfg, modifiers.ModifierCfg):
                            ...
                            if inspect.isclass(mod_cfg.func):
                                ...
                                self._group_obs_class_modifiers.append(
                                    mod_cfg.func)
                        else:
                            raise TypeError(
