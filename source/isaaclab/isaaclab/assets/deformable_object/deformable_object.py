...
class DeformableObject(AssetBase):
    ...
    #         collision_groups = {
    #             "rigidGroup": f"/World/envs/env_{i}/collisionGroupRigids",
    #             # "articulationGroup": "/physicsScene/collisionGroupParticles",
    #             "softbodyGroup":
    #             f"/World/envs/env_{i}/collisionGroupSoftbodies",
    #         }

    #         defined_groups = {}

    #         # Create and define the collision groups
    #         for group_name, group_path in collision_groups.items():
    #             defined_groups[group_name] = UsdPhysics.CollisionGroup.Define(
    #                 stage, group_path)

    #         # Set up filtered relationships so that groups do not collide with each other
    #         filtered_relationships = {
    #             "rigidGroup": ["softbodyGroup"],
    #             # "articulationGroup": ["rigidGroup", "softbodyGroup"],
    #             "softbodyGroup": ["rigidGroup"],
    #         }

    #         for group_name, filtered_targets in filtered_relationships.items():
    #             filtered_rel = defined_groups[
    #                 group_name].CreateFilteredGroupsRel()
    #             for target in filtered_targets:
    #                 filtered_rel.AddTarget(collision_groups[target])

    #         # Add collision groups
    #         from omni.physx.scripts import physicsUtils
    #         from pxr import Usd

    #         filter_paths = self.cfg.deform_cfg["filter_pair_path"]

    #         # Iterate over each path in filter_pair_path
    #         for j, filter_path in enumerate(filter_paths):
    #             # group_key = list(collision_groups.keys())[j]
    #             # physicsUtils.add_collision_to_collision_group(stage, filter_path.replace(".*", str(i)), collision_groups[group_key])

    #             physicsUtils.add_collision_to_collision_group(
    #                 stage, filter_path.replace(".*", str(i)),
    #                 collision_groups["rigidGroup"])

    #             collectionAPI = Usd.CollectionAPI.Get(
    #                 stage.GetPrimAtPath(collision_groups["rigidGroup"]),
    #                 "colliders")
    #             collectionAPI.GetExcludesRel().AddTarget(
    #                 self.mesh_path_expr.replace(".*", str(i)))

    #             collectionAPI = Usd.CollectionAPI.Get(
    #                 stage.GetPrimAtPath(collision_groups["softbodyGroup"]),
    #                 "colliders")
    #             collectionAPI.GetExcludesRel().AddTarget(
    #                 filter_path.replace(".*", str(i)))

    #             # Adding the softbody group separately as it is not in filter_pair_path
    #             physicsUtils.add_collision_to_collision_group(
    #                 stage, self.mesh_path_expr.replace(".*", str(i)),
    #                 collision_groups["softbodyGroup"])

    # # # Adding the softbody group separately as it is not in filter_pair_path
    # physicsUtils.add_collision_to_collision_group( stage, self.mesh_path_expr.replace(".*", str(i)),collision_groups["softbodyGroup"])

    # def pairfilter(self, ):

    #     # # if "filter_pair_path" in self.cfg.deform_cfg.keys():
    #     # stage = stage_utils.get_current_stage()
    #     # for i in range(self.num_instances):
    #     #     for target_filter_path in self.cfg.deform_cfg["filter_pair_path"]:
    #     #         taget_prim = stage.GetPrimAtPath(
    #     #             target_filter_path.replace(".*", str(i)))
    #     #         if taget_prim.IsValid():
    ...
    #     #             rel.AddTarget(
    #     #                 Sdf.Path(target_filter_path.replace(".*", str(i))))
    ...
    #     #             rel.AddTarget(
    #     #                 Sdf.Path(self.mesh_path_expr.replace(".*", str(i))))
    #     stage = stage_utils.get_current_stage()
    #     filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
    #         stage.GetPrimAtPath(self.mesh_path_expr.replace(".*", str(0))))
    #     rel = filteringPairsAPI.CreateFilteredPairsRel()
    #     rel.AddTarget(Sdf.Path("/World/envs/env_0/gripper"))
    #     rel.AddTarget(Sdf.Path("/World/envs/env_0/Robot"))

    #     filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
    #         stage.GetPrimAtPath("/World/envs/env_0/gripper"))
    #     rel = filteringPairsAPI.CreateFilteredPairsRel()
    #     rel.AddTarget(Sdf.Path(self.mesh_path_expr.replace(".*", str(0))))
    #     rel.AddTarget(Sdf.Path("/World/envs/env_0/Robot"))

    #     filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
    #         stage.GetPrimAtPath("/World/envs/env_0/Robot"))
    #     rel = filteringPairsAPI.CreateFilteredPairsRel()
    #     rel.AddTarget(Sdf.Path(self.mesh_path_expr.replace(".*", str(0))))
    #     rel.AddTarget(Sdf.Path("/World/envs/env_0/gripper"))
    ...
    def create_collision_filter(self):
        ...
        for i in range(self.num_instances):
            # Apply the FilteredPairsAPI to the primary mesh path
            primary_prim_path = self.mesh_path_expr.replace(".*", str(i))
            primary_prim = stage.GetPrimAtPath(primary_prim_path)

            if primary_prim.IsValid():
                filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
                    primary_prim)
                rel = filteringPairsAPI.CreateFilteredPairsRel()

                # Iterate over all filter paths
                for target_filter_path in self.cfg.deform_cfg[
                        "filter_pair_path"]:
                    target_prim_path = target_filter_path.replace(".*", str(i))
                    target_prim = stage.GetPrimAtPath(target_prim_path)

                    if target_prim.IsValid():
                        # Add the target prims to the primary prim's filtered pairs
                        rel.AddTarget(Sdf.Path(target_prim_path))

                        # # Apply the filtering in the opposite direction (bidirectional)
                        filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
                            target_prim)
                        rel_opposite = filteringPairsAPI.CreateFilteredPairsRel(
                        )
                        rel_opposite.AddTarget(Sdf.Path(primary_prim_path))

                        # Also, add all other filter paths to this target prim's filtered pairs
                        for other_filter_path in self.cfg.deform_cfg[
                                "filter_pair_path"]:
                            if other_filter_path != target_filter_path:
                                other_target_prim_path = other_filter_path.replace(
                                    ".*", str(i))
                                other_target_prim = stage.GetPrimAtPath(
                                    other_target_prim_path)
                                if other_target_prim.IsValid():
                                    rel_opposite.AddTarget(
                                        Sdf.Path(other_target_prim_path))
    ...
    def _initialize_impl(self):
        ...
        # if "filter_pair_path" in self.cfg.deform_cfg.keys():
        #     self.create_collision_filter()
        from isaaclab.managers.params_generator import ParamsGenerator
        if self.cfg.deform_cfg["random_parmas"]:
            if self.cfg.deform_cfg['physical_params']['params_values']:
                self.parames_generator = ParamsGenerator(
                    self.cfg.deform_cfg['physical_params']['params_values'],
                    self.cfg.deform_cfg['physical_params']['params_range'])
        else:
            self.parames_generator = None
            if "physical_params" in self.cfg.deform_cfg.keys():
                if self.cfg.deform_cfg['physical_params'][
                        'fix_params'] is not None:
                    self.random_physical_properties(
                        torch.arange(self.num_instances).to(self.device))
