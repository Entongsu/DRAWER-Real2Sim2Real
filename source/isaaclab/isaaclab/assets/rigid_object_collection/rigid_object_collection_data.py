...
class RigidObjectCollectionData:
    ...
    def object_state_w(self):
        ...
        if self._object_state_w.timestamp < self._sim_timestamp:
            ...
            velocity = self._reshape_view_to_data(
    ...
    def object_link_state_w(self):
        ...
        if self._object_link_state_w.timestamp < self._sim_timestamp:
            ...
            velocity[..., :3] += torch.linalg.cross(
                velocity[..., 3:],
                math_utils.quat_rotate(pose[..., 3:7],
                                       -self.com_pos_b[..., :]),
                dim=-1)

            # set the buffer data and timestamp
            self._object_link_state_w.data = torch.cat((pose, velocity),
                                                       dim=-1)
    ...
    def object_com_state_w(self):
        ...
        if self._object_com_state_w.timestamp < self._sim_timestamp:
            ...
            pos, quat = math_utils.combine_frame_transforms(
                pose[..., :3], pose[..., 3:7], self.com_pos_b[..., :],
                self.com_quat_b[..., :])

            # set the buffer data and timestamp
            self._object_com_state_w.data = torch.cat((pos, quat, velocity),
                                                      dim=-1)
    ...
    def object_acc_w(self):
        ...
        if self._object_acc_w.timestamp < self._sim_timestamp:
            ...
            self._object_acc_w.data = self._reshape_view_to_data(
    ...
    def projected_gravity_b(self):
        ...
        return math_utils.quat_rotate_inverse(self.object_link_quat_w,
                                              self.GRAVITY_VEC_W)
    ...
    def object_lin_vel_b(self) -> torch.Tensor:
        ...
        """
        return math_utils.quat_rotate_inverse(self.object_quat_w,
    ...
    def com_quat_b(self) -> torch.Tensor:
        ...
        """
        quat = self._root_physx_view.get_coms().to(self.device)[..., 3:7].view(
            self.num_instances, self.num_objects, 4)
