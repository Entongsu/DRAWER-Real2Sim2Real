...
class RigidObjectData:
    ...
    def root_link_state_w(self):
        ...
        if self._root_link_state_w.timestamp < self._sim_timestamp:
            ...
            velocity[:, :3] += torch.linalg.cross(
                velocity[:, 3:],
                math_utils.quat_rotate(pose[:, 3:7], -self.com_pos_b[:, 0, :]),
                dim=-1)
            # set the buffer data and timestamp
    ...
    def root_com_state_w(self):
        ...
        if self._root_com_state_w.timestamp < self._sim_timestamp:
            ...
            pos, quat = math_utils.combine_frame_transforms(
                pose[:, :3], pose[:, 3:7], self.com_pos_b[:, 0, :],
                self.com_quat_b[:, 0, :])
            pose = torch.cat((pos, quat), dim=-1)
            # set the buffer data and timestamp
            self._root_com_state_w.data = torch.cat((pos, quat, velocity),
                                                    dim=-1)
    ...
    def body_acc_w(self):
        ...
        if self._body_acc_w.timestamp < self._sim_timestamp:
            ...
            self._body_acc_w.data = self._root_physx_view.get_accelerations(
            ).unsqueeze(1)
    ...
    def projected_gravity_b(self):
        ...
        return math_utils.quat_rotate_inverse(self.root_link_quat_w,
                                              self.GRAVITY_VEC_W)
    ...
    def com_pos_b(self) -> torch.Tensor:
        ...
        """
        return self._root_physx_view.get_coms().to(self.device)[..., :3].view(
            -1, 1, 3)
