...
class RigidObject(AssetBase):
    ...
    def reset_default_root_state(self, target_root_state):

        from pxr import Gf
        import time
        env_ids = torch.arange(self.num_instances).to(self.device)

        for i, env_idx in enumerate(env_ids):

            prim_path = self.cfg.prim_path.replace(".*",
                                                   str(env_idx.cpu().numpy()))

            prim = self.stage.GetPrimAtPath(prim_path)
            # Define the block where stdout and stderr will be silenced

            result = prim.GetAttribute("xformOp:translate").Set(
                Gf.Vec3d(target_root_state[env_idx]
                         [:3].detach().cpu().numpy().tolist()))

            target_quat = target_root_state[env_idx][3:7].detach().cpu().numpy(
            ).tolist()
            # Ensure the imaginary part is passed as Gf.Vec3f, not Gf.Vec3d
            real_part = float(
                target_quat[0])  # The real part of the quaternion
            imaginary_part = Gf.Vec3f(
                float(target_quat[1]), float(target_quat[2]),
                float(target_quat[3]))  # Vec3f for the imaginary part

            # Set the quaternion using the correct types
            prim.GetAttribute("xformOp:orient").Set(
                Gf.Quatf(real_part, imaginary_part))

            del prim_path, prim, result
            gc.collect()
        default_root_state = self.data.default_root_state[env_ids].clone()
        self.write_root_state_to_sim(default_root_state, env_ids=env_ids)
    ...
    def write_root_velocity_to_sim(self,
        ...
        # set into simulation
        self.root_physx_view.set_velocities(self._data.root_state_w[:, 7:],
                                            indices=physx_env_ids)
    ...
    def write_root_com_velocity_to_sim(self,
        ...
        self._data.body_acc_w[env_ids] = 0.0
        # set into simulation
        self.root_physx_view.set_velocities(self._data.root_com_state_w[:, 7:],
                                            indices=physx_env_ids)
