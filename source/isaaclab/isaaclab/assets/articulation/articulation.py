...
class Articulation(AssetBase):
    ...
    def write_root_link_pose_to_sim(self,
        ...
        self._data.root_state_w[env_ids, :7] = self._data.root_link_state_w[
            env_ids, :7]
        # convert root quaternion from wxyz to xyzw
    ...
    def write_root_velocity_to_sim(self,
        ...
        # set into simulation
        self.root_physx_view.set_root_velocities(self._data.root_state_w[:,
                                                                         7:],
                                                 indices=physx_env_ids)
    ...
    def write_root_com_velocity_to_sim(self,
        ...
        self._data.root_state_w[env_ids,
                                7:] = self._data.root_com_state_w[env_ids, 7:]
        self._data.body_acc_w[env_ids] = 0.0
    ...
    def write_joint_state_to_sim(
        ...
        self.root_physx_view.set_dof_positions(self._data.reset_joint_pos,
                                               indices=physx_env_ids)
    ...
    def write_joint_velocity_limit_to_sim(
        ...
        # set into internal buffers
        self._data.joint_velocity_limits = self.root_physx_view.get_dof_max_velocities(
        ).to(self.device)
        self._data.joint_velocity_limits[env_ids, joint_ids] = limits
        # set into simulation
        self.root_physx_view.set_dof_max_velocities(
            self._data.joint_velocity_limits.cpu(),
            indices=physx_env_ids.cpu())
    ...
    def write_joint_effort_limit_to_sim(
        ...
        # set into simulation
        self.root_physx_view.set_dof_max_forces(torque_limit_all.cpu(),
                                                indices=physx_env_ids.cpu())
    ...
    def write_joint_limits_to_sim(
        ...
        # set into simulation
        self.root_physx_view.set_dof_limits(self._data.joint_limits.cpu(),
                                            indices=physx_env_ids.cpu())
    ...
    def _initialize_impl(self):
        ...
        #     physics_materials.spawn_rigid_body_material(
        #         self.cfg.prim_path.replace(".*", str(env_idx)) +
        #         "/panda_leftfinger/PhysicsMaterial", material_cfg)
        #     physics_materials.spawn_rigid_body_material(
        #         self.cfg.prim_path.replace(".*", str(env_idx)) +
        #         "/panda_rightfinger/PhysicsMaterial", material_cfg)
    ...
    def _create_buffers(self):
        ...
        # -- joint commands
        self._data.joint_pos_target = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_vel_target = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_effort_target = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_stiffness = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_damping = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_armature = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_friction = torch.zeros_like(
            self._data.default_joint_pos)
        self._data.joint_limits = torch.zeros(self.num_instances,
                                              self.num_joints,
                                              2,
                                              device=self.device)
        ...
        if self.num_fixed_tendons > 0:
            ...
            self._data.fixed_tendon_limit_stiffness = torch.zeros(
                self.num_instances, self.num_fixed_tendons, device=self.device)
            self._data.fixed_tendon_limit = torch.zeros(self.num_instances,
                                                        self.num_fixed_tendons,
                                                        2,
                                                        device=self.device)
        ...
        self._data.default_joint_stiffness = torch.zeros(self.num_instances,
                                                         self.num_joints,
                                                         device=self.device)
        self._data.default_joint_damping = torch.zeros(self.num_instances,
                                                       self.num_joints,
                                                       device=self.device)
        self._data.default_joint_armature = torch.zeros(self.num_instances,
                                                        self.num_joints,
                                                        device=self.device)
        self._data.default_joint_friction = torch.zeros(self.num_instances,
                                                        self.num_joints,
                                                        device=self.device)
        self._data.default_joint_limits = torch.zeros(self.num_instances,
                                                      self.num_joints,
                                                      2,
                                                      device=self.device)
        ...
        joint_pos_mean = (joint_pos_limits[..., 0] +
                          joint_pos_limits[..., 1]) / 2
    ...
    def _process_cfg(self):
        ...
        # -- joint limits
        self._data.default_joint_limits = self.root_physx_view.get_dof_limits(
        ).to(device=self.device).clone()
    ...
    def _process_actuators_cfg(self):
        ...
        self._data.default_joint_friction = self.root_physx_view.get_dof_friction_coefficients(
        ).to(self.device).clone()
        ...
        for actuator_name, actuator_cfg in self.cfg.actuators.items():
            ...
            actuator: ActuatorBase = actuator_cfg.class_type(
                ...
                joint_ids=(slice(None) if len(joint_names) == self.num_joints
                ...
                effort_limit=self.root_physx_view.get_dof_max_forces().to(
                    self.device).clone()[:, joint_ids],
                velocity_limit=self.root_physx_view.get_dof_max_velocities().
                to(self.device).clone()[:, joint_ids],
            )
    ...
    def _process_fixed_tendons(self):
        ...
        if self.num_fixed_tendons > 0:
            ...
            self._data.default_fixed_tendon_limit = self.root_physx_view.get_fixed_tendon_limits(
            ).clone()
    ...
    def _log_articulation_joint_info(self):
        ...
        frictions = self.root_physx_view.get_dof_friction_coefficients(
        )[0].tolist()
        # -- limits
