...
def reset_deform(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Terminate the episode when the episode length exceeds the maximum episode length."""
    if env.episode_length_buf < 0:

        object_assest = env.scene["deform_object"]
        object_assest.root_physx_view.set_sim_nodal_positions(
            object_assest._data.
            default_nodal_state_w[:, :object_assest.
                                  max_simulation_mesh_vertices_per_body, :],
            indices=torch.range(0, object_assest.num_instances - 1).to(
                object_assest._data.default_nodal_state_w.device))
        object_assest.root_physx_view.set_sim_nodal_velocities(
            object_assest._data.
            nodal_state_w[:, object_assest.
                          max_simulation_mesh_vertices_per_body:, :] * 0,
            indices=torch.range(0, object_assest.num_instances - 1).to(
                object_assest._data.default_nodal_state_w.device))
...
def joint_pos_out_of_limit(
    ...
    out_of_lower_limits = torch.any(asset.data.joint_pos
                                    ...
                                    dim=1)
    return torch.logical_or(out_of_upper_limits[:, asset_cfg.joint_ids],
