...
def randomize_rigid_body_mass(
    ...
    masses[env_ids[:, None],
           body_ids] = asset.data.default_mass[env_ids[:, None],
                                               body_ids].clone()
...
def randomize_actuator_gains(
    ...
    def randomize(data: torch.Tensor, params: tuple[float,
                                                    float]) -> torch.Tensor:
    ...
    for actuator in asset.actuators.values():
        if isinstance(asset_cfg.joint_ids, slice):
            ...
            if isinstance(actuator.joint_indices, slice):
                ...
                global_indices = torch.tensor(actuator.joint_indices,
                                              device=asset.device)
            ...
            actuator_joint_indices = torch.tensor(actuator.joint_indices,
                                                  device=asset.device)
            asset_joint_ids = torch.tensor(asset_cfg.joint_ids,
                                           device=asset.device)
            # the indices of the joints in the actuator that have to be randomized
            actuator_indices = torch.nonzero(
                torch.isin(actuator_joint_indices, asset_joint_ids)).view(-1)
        ...
        if stiffness_distribution_params is not None:
            ...
            stiffness[:,
                      actuator_indices] = asset.data.default_joint_stiffness[
        ...
        if damping_distribution_params is not None:
            ...
            damping[:, actuator_indices] = asset.data.default_joint_damping[
                env_ids][:, global_indices].clone()
...
def randomize_joint_parameters(
    ...
    if asset_cfg.joint_ids == slice(None):
        ...
        joint_ids = torch.tensor(asset_cfg.joint_ids,
                                 dtype=torch.int,
                                 device=asset.device)
    ...
    if friction_distribution_params is not None:
        ...
        friction = _randomize_prop_by_op(
            ...
            joint_ids,
            operation=operation,
            distribution=distribution)[env_ids][:, joint_ids]
        asset.write_joint_friction_to_sim(friction,
                                          joint_ids=joint_ids,
                                          env_ids=env_ids)
    # -- armature
    if armature_distribution_params is not None:
        ...
        armature = _randomize_prop_by_op(
            ...
            operation=operation,
            distribution=distribution)[env_ids][:, joint_ids]
        asset.write_joint_armature_to_sim(armature,
                                          joint_ids=joint_ids,
                                          env_ids=env_ids)
    # -- dof limits
    if lower_limit_distribution_params is not None or upper_limit_distribution_params is not None:
        ...
        if (dof_limits[env_ids[:, None], joint_ids, 0]
                > dof_limits[env_ids[:, None], joint_ids, 1]).any():
            raise ValueError(
                "Randomization term 'randomize_joint_parameters' is setting lower joint limits that are greater than"
                " upper joint limits.")

        asset.write_joint_limits_to_sim(dof_limits[env_ids][:, joint_ids],
                                        joint_ids=joint_ids,
                                        env_ids=env_ids,
...
def randomize_fixed_tendon_parameters(
    ...
    if asset_cfg.fixed_tendon_ids == slice(None):
        ...
        fixed_tendon_ids = torch.tensor(asset_cfg.fixed_tendon_ids,
                                        dtype=torch.int,
                                        device=asset.device)
    ...
    if limit_stiffness_distribution_params is not None:
        ...
        asset.set_fixed_tendon_limit_stiffness(limit_stiffness,
    ...
    if lower_limit_distribution_params is not None or upper_limit_distribution_params is not None:
        ...
        if (limit[env_ids[:, None], fixed_tendon_ids, 0]
                > limit[env_ids[:, None], fixed_tendon_ids, 1]).any():
            raise ValueError(
    ...
    if rest_length_distribution_params is not None:
        ...
        asset.set_fixed_tendon_rest_length(rest_length, fixed_tendon_ids,
                                           env_ids)
    # -- offset
...
def attact_robot_deform(env: ManagerBasedEnv, env_ids: torch.Tensor):
    object = env.scene["deform_object"]
    ...
    if object.cfg.deform_cfg["attach_robot"][
        ...
        for i in env_ids.cpu().numpy():
            ...
            # autoApi.GetRigidSurfaceSamplingDistanceAttr().Set(1)
...
def reset_scene_to_default(env: ManagerBasedEnv, env_ids: torch.Tensor):
    ...
    for rigid_object in env.scene.rigid_objects.values():
        ...
        default_root_state = rigid_object.data.reset_root_state[env_ids].clone(
        )
    ...
    for articulation_asset in env.scene.articulations.values():
        # obtain default and deal with the offset for env origins
        default_root_state = articulation_asset.data.reset_root_state[
            env_ids].clone()
        default_root_state[:, 0:3] += env.scene.env_origins[env_ids]
        # set into the physics simulation
        ...
        default_joint_pos = articulation_asset.data.reset_joint_pos[
            env_ids].clone()
...
def add_noise_to_position(ee_pose, position_range):
    ...
    ee_pose[:, 3:7] = math_utils.quat_mul(delta_quat_noise, ee_pose[:, 3:7])

    return ee_pose


def robottip_pose(env, use_gripper_offset: bool = True):
    ...
    if use_gripper_offset:
        ee_pose_b, ee_quat_b = math_utils.combine_frame_transforms(
            ...
            torch.tensor([[1., 0., 0., 0.]],
                         device=env.device).repeat_interleave(env.num_envs, 0))
    gripper = torch.sign(env.scene["robot"]._data.joint_pos[:, -1][:, None] -
                         0.03)
