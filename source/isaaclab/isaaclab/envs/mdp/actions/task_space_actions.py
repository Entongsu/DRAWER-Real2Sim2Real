...
class DifferentialInverseKinematicsAction(ActionTerm):
    ...
    def apply_actions(self):
        ...
        if ee_quat_curr.norm() != 0:
            ...
            self.joint_pos_des = self._ik_controller.compute(
                ee_pos_curr, ee_quat_curr, jacobian, joint_pos)
        else:
            self.joint_pos_des = joint_pos.clone()
        # set the joint position command

        self._asset.set_joint_position_target(self.joint_pos_des,
                                              self._joint_ids)
...
class OperationalSpaceControllerAction(ActionTerm):
    ...
    def _compute_dynamic_quantities(self):
        ...
        self._mass_matrix[:] = self._asset.root_physx_view.get_mass_matrices(
        )[:, self._joint_ids, :][:, :, self._joint_ids]
        self._gravity[:] = self._asset.root_physx_view.get_generalized_gravity_forces(
        )[:, self._joint_ids]
    ...
    def _compute_ee_pose(self):
        ...
        # Obtain quantities from simulation
        self._ee_pose_w[:,
                        0:3] = self._asset.data.body_link_pos_w[:, self.
                                                                _ee_body_idx]
        self._ee_pose_w[:,
                        3:7] = self._asset.data.body_link_quat_w[:, self.
                                                                 _ee_body_idx]
    ...
    def _compute_ee_velocity(self):
        ...
        self._ee_vel_w[:] = self._asset.data.body_com_vel_w[:, self.
                                                            _ee_body_idx, :]
    ...
    def _compute_ee_force(self):
        ...
        if self._contact_sensor is not None:
            ...
            self._ee_force_b[:] = math_utils.quat_rotate_inverse(
                self._asset.data.root_link_quat_w, self._ee_force_w)
