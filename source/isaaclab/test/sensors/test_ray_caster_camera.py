...
class TestWarpCamera(unittest.TestCase):
    ...
    def test_camera_init(self):
        ...
        self.assertEqual(camera.data.image_shape,
                         (self.camera_cfg.pattern_cfg.height,
                          self.camera_cfg.pattern_cfg.width))
        ...
        for _ in range(10):
            ...
            for im_data in camera.data.output.values():
                self.assertEqual(im_data.shape,
                                 (1, self.camera_cfg.pattern_cfg.height,
                                  self.camera_cfg.pattern_cfg.width, 1))
    ...
    def test_camera_resolution(self):
        ...
        for im_data in camera.data.output.values():
            self.assertTrue(
                im_data.shape == (1, self.camera_cfg.pattern_cfg.height,
                                  self.camera_cfg.pattern_cfg.width, 1))
    ...
    def test_throughput(self):
        ...
        for _ in range(5):
            ...
            for im_data in camera.data.output.values():
                self.assertEqual(im_data.shape,
                                 (1, camera_cfg.pattern_cfg.height,
    ...
    def test_output_equal_to_usdcamera_prim_offset(self):
        ...
        prim_raycast_cam = prim_utils.create_prim("/World/Camera_warp",
                                                  "Xform")
    ...
    def test_output_equal_to_usd_camera_intrinsics(self):
        ...
        cam_warp_output = camera_warp.data.output[
            "distance_to_image_plane"].clone()
        cam_usd_output = camera_usd.data.output[
            "distance_to_image_plane"].clone()
    ...
    def test_output_equal_to_usd_camera_when_intrinsics_set(self):
        ...
        intrinsic_matrix = torch.tensor(
            [[
                380.0831, 0.0, camera_cfg_usd.width / 2, 0.0, 380.0831,
                camera_cfg_usd.height / 2, 0.0, 0.0, 1.0
            ]],
            device=camera_warp.device,
        ).reshape(1, 3, 3)
