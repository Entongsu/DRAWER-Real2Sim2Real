...
def quat_from_euler_rpy(roll, pitch, yaw, degrees=False):
    ...
    quat = tf.Rotation.from_euler("xyz", (roll, pitch, yaw),
                                  degrees=degrees).as_quat()
...
class TestFrameTransformer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sim = sim_utils.SimulationContext(
            sim_utils.SimulationCfg(dt=0.005, device="cpu"))
        # Set main camera
    ...
    def test_frame_transformer_feet_wrt_base(self):
        ...
        scene_cfg = MySceneCfg(num_envs=32,
                               env_spacing=5.0,
        ...
        scene_cfg.frame_transformer = FrameTransformerCfg(
            ...
            target_frames=[
                FrameTransformerCfg.FrameCfg(
                    ...
                    offset=OffsetCfg(
                        pos=euler_rpy_apply(rpy=(0, 0, -math.pi / 2),
                                            xyz=(0.08795, 0.01305, -0.33797)),
        ...
        for count in range(100):
            ...
            if count % 25 == 0:
                ...
                root_state = scene.articulations[
                    "robot"].data.default_root_state.clone()
                ...
                # -- set root state
                # -- robot
                scene.articulations["robot"].write_root_pose_to_sim(
                    root_state[:, :7])
            ...
            # set joint targets
            robot_actions = default_actions + 0.5 * torch.randn_like(
                default_actions)
            ...
            feet_pos_w_gt = scene.articulations[
                "robot"].data.body_pos_w[:, feet_indices]
            feet_quat_w_gt = scene.articulations[
                "robot"].data.body_quat_w[:, feet_indices]
            # -- frame transformer
            source_pos_w_tf = scene.sensors[
                "frame_transformer"].data.source_pos_w
            source_quat_w_tf = scene.sensors[
                "frame_transformer"].data.source_quat_w
            feet_pos_w_tf = scene.sensors[
                "frame_transformer"].data.target_pos_w
            feet_quat_w_tf = scene.sensors[
            ...
            feet_pos_source_tf = scene.sensors[
                "frame_transformer"].data.target_pos_source
            feet_quat_source_tf = scene.sensors[
            ...
            for index in range(len(feet_indices)):
                ...
                foot_pos_b, foot_quat_b = math_utils.subtract_frame_transforms(
                    ...
                    feet_pos_w_tf[:, index], feet_quat_w_tf[:, index])
                # check if they are same
                torch.testing.assert_close(feet_pos_source_tf[:, index],
                                           foot_pos_b)
                torch.testing.assert_close(feet_quat_source_tf[:, index],
                                           foot_quat_b)
    ...
    def test_frame_transformer_feet_wrt_thigh(self):
        ...
        # Acquire the index of ground truth bodies
        source_frame_index = scene.articulations["robot"].find_bodies(
            "LF_THIGH")[0][0]
        feet_indices, feet_names = scene.articulations["robot"].find_bodies(
            ["LF_FOOT", "RF_FOOT"])
        ...
        self.assertListEqual(
            scene.sensors["frame_transformer"].data.target_frame_names,
            user_feet_names)
        ...
        for count in range(100):
            ...
            # check absolute frame transforms in world frame
            # -- ground-truth
            source_pose_w_gt = scene.articulations[
                "robot"].data.body_state_w[:, source_frame_index, :7]
            feet_pos_w_gt = scene.articulations[
                "robot"].data.body_pos_w[:, feet_indices]
            feet_quat_w_gt = scene.articulations[
                "robot"].data.body_quat_w[:, feet_indices]
            ...
            feet_quat_w_tf = scene.sensors[
                "frame_transformer"].data.target_quat_w
            # check if they are same
            torch.testing.assert_close(source_pose_w_gt[:, :3],
                                       source_pos_w_tf)
            torch.testing.assert_close(source_pose_w_gt[:, 3:],
                                       source_quat_w_tf)
            torch.testing.assert_close(feet_pos_w_gt, feet_pos_w_tf)
            ...
            for index in range(len(feet_indices)):
                # ground-truth
                foot_pos_b, foot_quat_b = math_utils.subtract_frame_transforms(
                    source_pose_w_gt[:, :3], source_pose_w_gt[:, 3:],
                    feet_pos_w_tf[:, index], feet_quat_w_tf[:, index])
                # check if they are same
                torch.testing.assert_close(feet_pos_source_tf[:, index],
                                           foot_pos_b)
    ...
    def test_frame_transformer_robot_body_to_external_cube(self):
        ...
        for count in range(100):
            ...
            # -- ground-truth
            root_pose_w = scene.articulations["robot"].data.root_state_w[:, :7]
            cube_pos_w_gt = scene.rigid_objects[
                "cube"].data.root_state_w[:, :3]
            cube_quat_w_gt = scene.rigid_objects["cube"].data.root_state_w[:,
                                                                           3:7]
            # -- frame transformer
            source_pos_w_tf = scene.sensors[
                "frame_transformer"].data.source_pos_w
            source_quat_w_tf = scene.sensors[
                "frame_transformer"].data.source_quat_w
            cube_pos_w_tf = scene.sensors[
                "frame_transformer"].data.target_pos_w.squeeze()
            cube_quat_w_tf = scene.sensors[
                "frame_transformer"].data.target_quat_w.squeeze()
            ...
            # check if relative transforms are same
            cube_pos_source_tf = scene.sensors[
                "frame_transformer"].data.target_pos_source
            cube_quat_source_tf = scene.sensors[
                "frame_transformer"].data.target_quat_source
            # ground-truth
            cube_pos_b, cube_quat_b = math_utils.subtract_frame_transforms(
                root_pose_w[:, :3], root_pose_w[:, 3:], cube_pos_w_tf,
                cube_quat_w_tf)
            # check if they are same
    ...
    def test_frame_transformer_offset_frames(self):
        ...
        for count in range(100):
            ...
            source_quat_w_tf = scene.sensors[
                "frame_transformer"].data.source_quat_w
            target_pos_w_tf = scene.sensors[
                "frame_transformer"].data.target_pos_w.squeeze()
            target_quat_w_tf = scene.sensors[
                "frame_transformer"].data.target_quat_w.squeeze()
            target_frame_names = scene.sensors[
                "frame_transformer"].data.target_frame_names
            ...
            torch.testing.assert_close(cube_pos_w_gt,
                                       target_pos_w_tf[:, cube_center_idx])
            torch.testing.assert_close(cube_quat_w_gt,
                                       target_quat_w_tf[:, cube_center_idx])
            ...
            torch.testing.assert_close(
                cube_pos_top, cube_pos_w_gt + torch.tensor([0.0, 0.0, 0.1]))
            ...
            torch.testing.assert_close(
                cube_pos_bottom,
                cube_pos_w_gt + torch.tensor([0.0, 0.0, -0.1]))
    ...
    def test_frame_transformer_all_bodies(self):
        ...
        reordering_indices = [
            target_frame_names.index(name) for name in articulation_body_names
        ]
        ...
        for count in range(100):
            ...
            source_quat_w_tf = scene.sensors[
                "frame_transformer"].data.source_quat_w
            bodies_pos_w_tf = scene.sensors[
                "frame_transformer"].data.target_pos_w
            bodies_quat_w_tf = scene.sensors[
                "frame_transformer"].data.target_quat_w

            # check if they are same
            torch.testing.assert_close(root_pose_w[:, :3], source_pos_w_tf)
            torch.testing.assert_close(root_pose_w[:, 3:], source_quat_w_tf)
            torch.testing.assert_close(bodies_pos_w_gt,
                                       bodies_pos_w_tf[:, reordering_indices])
            torch.testing.assert_close(bodies_quat_w_gt,
                                       bodies_quat_w_tf[:, reordering_indices])

            bodies_pos_source_tf = scene.sensors[
                "frame_transformer"].data.target_pos_source
            bodies_quat_source_tf = scene.sensors[
                "frame_transformer"].data.target_quat_source
            ...
            for index in range(len(articulation_body_names)):
                body_pos_b, body_quat_b = math_utils.subtract_frame_transforms(
                    root_pose_w[:, :3], root_pose_w[:, 3:],
                    bodies_pos_w_tf[:, index], bodies_quat_w_tf[:, index])

                torch.testing.assert_close(bodies_pos_source_tf[:, index],
                                           body_pos_b)
                torch.testing.assert_close(bodies_quat_source_tf[:, index],
                                           body_quat_b)
