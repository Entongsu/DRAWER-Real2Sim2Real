...
class TestContactSensor(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sim_dt = 0.0025
        cls.durations = [
            cls.sim_dt, cls.sim_dt * 2, cls.sim_dt * 32, cls.sim_dt * 128
        ]
            ...
            for num_envs in [1, 6, 24]:
                with self.subTest(device=device, num_envs=num_envs):
                    with build_simulation_context(device=device,
                        ...
                        scene_cfg.terrain = FLAT_TERRAIN_CFG.replace(
                            prim_path="/World/ground")
                        # -- cube 1
                        ...
                        scene_cfg.shape_2 = CUBE_CFG.replace(
                        ...
                        contact_sensor: ContactSensor = self.scene[
                            "contact_sensor"]
                        contact_sensor_2: ContactSensor = self.scene[
                            "contact_sensor_2"]
                        # Check buffers have the right size
                        self.assertEqual(
                            contact_sensor.contact_physx_view.filter_count, 1)
                        self.assertEqual(
                            contact_sensor_2.contact_physx_view.filter_count,
                            1)
    ...
    def _run_contact_sensor_test(self,
    ...
    def _test_sensor_contact(self, shape: RigidObject, sensor: ContactSensor,
        ...
        for idx in range(len(self.durations)):
            ...
            expected_last_test_contact_time = self.durations[
                idx - 1] + self.sim_dt if idx > 0 else 0
    ...
    def _check_prim_contact_state_times(
        ...
        self.assertAlmostEqual(measured_contact_time.item(),
                               expected_contact_time,
                               places=2)
        self.assertAlmostEqual(measured_air_time.item(),
                               expected_air_time,
                               places=2)
        # check last contact state
        self.assertAlmostEqual(measured_last_contact_time.item(),
                               expected_last_contact_time,
                               places=2)
        self.assertAlmostEqual(measured_last_air_time.item(),
                               expected_last_air_time,
                               places=2)
        # check current contact mode
        self.assertEqual(
            sensor.compute_first_contact(dt=dt).item(), in_contact)
