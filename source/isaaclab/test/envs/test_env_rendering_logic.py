...
class TestEnvRenderingLogic(unittest.TestCase):
    ...
    def test_env_rendering_logic(self):
        for env_type in [
                "manager_based_env", "manager_based_rl_env", "direct_rl_env"
        ]:
                ...
                with self.subTest(env_type=env_type,
                    ...
                    try:
                        ...
                        if "env" in locals() and hasattr(env, "_is_closed"):
                            ...
                            if hasattr(e, "obj") and hasattr(
                                    e.obj, "_is_closed"):
                                e.obj.close()
                        self.fail(
                            f"Failed to set-up the environment {env_type}. Error: {e}"
                        )
                    ...
                    self.assertEqual(
                        env.sim.render_mode,
                        SimulationContext.RenderMode.PARTIAL_RENDERING)

                    # add physics and render callbacks
                    env.sim.add_physics_callback("physics_step",
                                                 self._physics_callback)
                    env.sim.add_render_callback("render_step",
                    ...
                    for i in range(50):
                        ...
                        self.assertEqual(self.num_physics_steps,
                                         (i + 1) * env.cfg.decimation,
                                         msg="Physics steps mismatch")
                        # check that we have simulated physics for the correct amount of time
                        self.assertAlmostEqual(self.physics_time,
                                               self.num_physics_steps *
                                               env.cfg.sim.dt,
                                               msg="Physics time mismatch")
                        ...
                        self.assertAlmostEqual(
                            self.render_time,
                            self.num_render_steps * env.cfg.sim.dt *
                            env.cfg.sim.render_interval,
