                ...
                with self.subTest(test_config=test_config, device=device):
                    with build_simulation_context(device=device) as sim:
                        ...
                        target_paths = cloner.generate_paths(
                            "/World/Robots", 4096)
                        omni.usd.get_context().get_stage().DefinePrim(
                            target_paths[0], "Xform")
                        _ = cloner.clone(
                        ...
                        with Timer(
                                f"{test_config[0]} load time for device {device}"
                        ) as timer:
                            robot = Articulation(  # noqa: F841
                                test_config[1]["robot_cfg"].replace(
                                    prim_path="/World/Robots_.*/Robot"))
                            sim.reset()
                            elapsed_time = timer.time_elapsed
                        self.assertLessEqual(
                            elapsed_time, test_config[1]["expected_load_time"])
