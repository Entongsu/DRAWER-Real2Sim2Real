...
class TestDifferentialIKController(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ee_pose_b_des_set = torch.tensor(ee_goals_set,
    ...
    def test_franka_ik_pose_abs(self):
        ...
        self._run_ik_controller(robot, diff_ik_controller, "panda_hand",
                                ["panda_joint.*"])
    ...
    def _run_ik_controller(
        ...
        ee_pose_b_des = torch.zeros(self.num_envs,
        ...
        for count in range(1500):
            ...
            if count % 250 == 0:
                ...
                root_state[:, 3:7] = random_yaw_orientation(
                ...
                current_goal_idx = (current_goal_idx + 1) % len(
                    self.ee_pose_b_des_set)
                ...
                jacobian[:, :3, :] = torch.bmm(base_rot_matrix,
                                               jacobian[:, :3, :])
                jacobian[:, 3:, :] = torch.bmm(base_rot_matrix,
                                               jacobian[:, 3:, :])
