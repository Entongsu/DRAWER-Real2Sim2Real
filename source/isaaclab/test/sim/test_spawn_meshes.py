...
class TestSpawningMeshGeometries(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dt = 0.1
        # Load kit helper
        self.sim = SimulationContext(physics_dt=self.dt,
    ...
    def test_spawn_cone_with_deformable_props(self):
        ...
        prim = prim_utils.get_prim_at_path("/World/Cone/geometry/mesh")
        self.assertEqual(
            prim.GetAttribute("physxDeformable:deformableEnabled").Get(),
    ...
    def test_spawn_cone_with_deformable_and_mass_props(self):
        ...
        prim = prim_utils.get_prim_at_path("/World/Cone/geometry/mesh")
        self.assertEqual(
            prim.GetAttribute("physics:mass").Get(), cfg.mass_props.mass)
    ...
    def test_spawn_cone_with_deformable_and_density_props(self):
        ...
        prim = prim_utils.get_prim_at_path("/World/Cone/geometry/mesh")
        self.assertEqual(
            prim.GetAttribute("physics:density").Get(), cfg.mass_props.density)
        # check sim playing
        self.sim.play()
        for _ in range(10):
            self.sim.step()
    ...
    def test_spawn_cone_with_all_deformable_props(self):
        ...
        self.assertTrue(prim_utils.is_prim_path_valid("/World/Cone"))
        self.assertTrue(
            prim_utils.is_prim_path_valid("/World/Cone/geometry/material"))
        # Check properties
        # -- deformable body
        prim = prim_utils.get_prim_at_path("/World/Cone/geometry/mesh")
        self.assertEqual(
            prim.GetAttribute("physxDeformable:deformableEnabled").Get(), True)
    ...
    def test_spawn_cone_with_all_rigid_props(self):
        ...
        self.assertEqual(
            prim.GetAttribute("physics:rigidBodyEnabled").Get(),
            cfg.rigid_props.rigid_body_enabled)
        self.assertEqual(
            prim.GetAttribute(
                "physxRigidBody:solverPositionIterationCount").Get(),
            cfg.rigid_props.solver_position_iteration_count,
        )
        self.assertAlmostEqual(
            prim.GetAttribute("physxRigidBody:sleepThreshold").Get(),
        ...
        prim = prim_utils.get_prim_at_path("/World/Cone/geometry/mesh")
        self.assertEqual(
            prim.GetAttribute("physics:collisionEnabled").Get(), True)
    ...
    def test_spawn_cone_clones_invalid_paths(self):
        ...
        for i in range(num_clones):
            prim_utils.create_prim(f"/World/env_{i}",
                                   ...
                                   translation=(i, i, 0))
        # Spawn cone
        cfg = sim_utils.MeshConeCfg(radius=1.0,
                                    height=2.0,
