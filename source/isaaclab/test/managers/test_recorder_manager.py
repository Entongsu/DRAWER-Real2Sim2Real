...
def create_dummy_env(device: str = "cpu") -> ManagerBasedEnv:
    ...
    return namedtuple("ManagerBasedEnv",
                      ["num_envs", "device", "cfg", "termination_manager"])(
...
class TestRecorderManager(unittest.TestCase):
    ...
    def test_initialize_dataset_file(self):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(cfg.dataset_export_dir_path,
        ...
        for device in ("cuda:0", "cpu"):
            with self.subTest(device=device):
                ...
                recorder_manager = RecorderManager(
                    self.create_dummy_recorder_manager_cfg(), env)
                ...
                for env_id in range(env.num_envs):
                    ...
                    self.assertEqual(episode.data["record_pre_step"].shape,
                                     (2, 4))
                    self.assertEqual(episode.data["record_post_step"].shape,
                                     (2, 5))
                ...
                for env_id in range(env.num_envs):
                    episode = recorder_manager.get_episode(env_id)
                    self.assertEqual(episode.data["record_post_reset"].shape,
                                     (1, 3))
