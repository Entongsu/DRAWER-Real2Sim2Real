...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg,
    ...
    collection_fps = (1 / (np.array(log_data["Perf/collection time"])) *
                      env.unwrapped.num_envs * agent_cfg.num_steps_per_env)
    rl_training_times = {
        "Collection Time":
    ...
    log_simulation_start_time(benchmark,
                              Timer.get_timer_info("simulation_start") * 1000)
    log_total_start_time(benchmark,
                         (task_startup_time_end - app_start_time_begin) / 1e6)
