...
def main():
    ...
    if not args_cli.auto and len(args_cli.signals) == 0:
        if len(args_cli.signals) == 0:
            raise ValueError(
                "Subtask signals should be provided for manual mode.")
    ...
    # Only enables inputs if this script is NOT headless mode
    if not args_cli.headless and not os.environ.get("HEADLESS", 0):
        keyboard_interface = Se3Keyboard(pos_sensitivity=0.1,
                                         rot_sensitivity=0.1)
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running() and not simulation_app.is_exiting():
            ...
            for episode_index, episode_name in enumerate(
                ...
                for action_index, action in enumerate(actions):
                    ...
                    if first_action:
                        ...
                        while is_paused:
                            ...
                            continue
                    action_tensor = torch.Tensor(action).reshape(
                        [1, action.shape[0]])
                ...
                if not args_cli.auto:
                    print(
                        f"\tSubtasks marked at action indices: {subtask_indices}"
                    )
                    ...
                    annotated_episode = env.unwrapped.recorder_manager.get_episode(
                        0)
                    for subtask_index in range(len(args_cli.signals)):
                        ...
                        subtask_signals = torch.ones(len(actions),
                                                     dtype=torch.bool)
                        subtask_signals[:
                                        subtask_indices[subtask_index]] = False
