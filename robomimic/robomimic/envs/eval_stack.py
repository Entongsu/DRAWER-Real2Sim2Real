...
def process_image(image, image_name="frontview_image"):
    ...
    x_min, x_max, y_min, y_max = 0, None, 80, -80

    obs0[image_name] = obs0[image_name][x_min:x_max, y_min:y_max]
...
if __name__ == "__main__":
    ...
    device = "cuda:0"
    ckpt_path = "/media/lme/data2/weird/robomimic/bc_trained_models/red_block_pick/20240531102548/models/model_epoch_200.pth"
    file_name = '/media/lme/data4/data_collection/0529/red_block_pick/demos.hdf5'
    
    stats = pickle.load(
        open(
            os.path.join(
                '/media/lme/data4/data_collection/0529/red_block_pick/',
                "stats"), "rb"))
    if args.use_rgb:
        camera_heights = 320
        camera_widths = 640
    ...
    cameras_dict = read_camera_data(
        "/media/lme/data2/weird/robosuite/robosuite/caliberation/camera.pkl")
    ...
    for num in range(num_traj):
        ...
        if env.use_camera_obs and args.use_rgb:
            ...
            obs0["front_rgb"] = processed_image
        ...
        for index in range(horizon):
            if args.mode == "train":
                ...
                if args.use_rgb:
                    observaion["front_rgb"] = obs["front_rgb"][
                        index]

                    observaion[
                        "front_rgb"] = transformed_image = np.transpose(
                            observaion["front_rgb"], (2, 0, 1)) / 255
                ...
                ac[-1] = np.sign(ac[-1])
            ac = unnormalize(ac, stats["action"])
            ...
            # env.render()
            if args.mode == "train_obs":
                cv2.imshow('Real-time video',
                            cv2.cvtColor(obs["front_rgb"][
                            index] , cv2.COLOR_BGR2RGB))
            else:
            
                cv2.imshow('Real-time video',
                            cv2.cvtColor((processed_image*255).astype(np.uint8), cv2.COLOR_BGR2RGB))
                # env.render()
