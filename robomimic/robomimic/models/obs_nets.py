    ...
    for k, obs_shape in obs_shapes.items():
        ...
        for obs_module, cls_mapping in zip(
            ...
            if enc_kwargs[f"{obs_module}_class"] is not None:
                enc_kwargs[
                    f"{obs_module}_kwargs"] = extract_class_init_kwargs_from_dict(
...
class ObservationEncoder(Module):
        ...
        for k in self.obs_shapes:
            ...
            if self.obs_nets_classes[k] is not None:
                ...
                self.obs_nets[k] = ObsUtils.OBS_ENCODER_CORES[
                    self.obs_nets_classes[k]](**self.obs_nets_kwargs[k])
    ...
    def _get_vis_lang_info(self):
        ...
        if (len(rgb_inds_need_lang_cond)
                > 0) or LangUtils.LANG_VIS_MULT_ENABLED:
    ...
    def forward(self, obs_dict):
        ...
        for ind, k in enumerate(self.obs_shapes):
            ...
            if LangUtils.LANG_COND_ENABLED and (not include_lang_feat) and (
                    ind in lang_inds):
                continue
            if k == "imagin_robot":
                continue
            if k == "seg_pc" and "imagin_robot" in obs_dict.keys():
                x = torch.cat([
                    obs_dict["seg_pc"][:, :3], obs_dict["imagin_robot"][:, :3]
                ],
                              dim=-1)
            else:
                x = obs_dict[k]
            ...
            if self.obs_nets[k] is not None:
                if LangUtils.LANG_COND_ENABLED and (
                        ind in rgb_inds_need_lang_cond):
    ...
    def output_shape(self, input_shape=None):
        ...
        for k in self.obs_shapes:
            ...
            if self.obs_randomizers[k] is not None:
                feat_shape = self.obs_randomizers[k].output_shape_in(
                    feat_shape)
            if self.obs_nets[k] is not None:
                feat_shape = self.obs_nets[k].output_shape(feat_shape)
            if self.obs_randomizers[k] is not None:
                feat_shape = self.obs_randomizers[k].output_shape_out(
                    feat_shape)
    ...
    def __repr__(self):
        ...
        for k in self.obs_shapes:
            ...
            msg += textwrap.indent(
                "modality={}\n".format(ObsUtils.OBS_KEYS_TO_MODALITIES[k]),
                indent)
            msg += textwrap.indent(
    ...
    def forward(self, **inputs):
        ...
        assert set(self.observation_group_shapes.keys()).issubset(
            inputs), "{} does not contain all observation groups {}".format(
                list(inputs.keys()),
                list(self.observation_group_shapes.keys()))
...
class MIMO_MLP(Module):
    ...
    def __init__(
        ...
        if LangUtils.LANG_COND_ENABLED:
            mlp_input_dim -= self.input_obs_group_shapes["obs"][
        ...
        self.nets["mlp"] = MLP(
            ...
            output_activation=
            activation,  # make sure non-linearity is applied before decoder
        )
    ...
    def __repr__(self):
        ...
        if self._to_string() != '':
            msg += textwrap.indent("\n" + self._to_string() + "\n", indent)
        msg += textwrap.indent("\nencoder={}".format(self.nets["encoder"]),
                               indent)
        msg += textwrap.indent("\n\nmlp={}".format(self.nets["mlp"]), indent)
        msg += textwrap.indent("\n\ndecoder={}".format(self.nets["decoder"]),
                               indent)
        msg = header + '(' + msg + '\n)'
        return msg
...
class RNN_MIMO_MLP(Module):
    ...
    def __init__(
        ...
        # flat encoder output dimension
        if LangUtils.LANG_COND_ENABLED:
            rnn_input_dim -= self.input_obs_group_shapes["obs"][
                LangUtils.LANG_OBS_KEY][0]
        ...
        num_directions = int(
            rnn_is_bidirectional) + 1  # 2 if bidirectional, 1 otherwise
        rnn_output_dim = num_directions * rnn_hidden_dim

        per_step_net = None
        self._has_mlp = (len(mlp_layer_dims) > 0)
        if self._has_mlp:
            self.nets["mlp"] = MLP(input_dim=rnn_output_dim,
            ...
            if self.per_step:
                per_step_net = Sequential(self.nets["mlp"],
                                          self.nets["decoder"])
        else:
            self.nets["decoder"] = ObservationDecoder(
                decode_shapes=self.output_shapes,
        ...
        # core network
        self.nets["rnn"] = RNN_Base(input_dim=rnn_input_dim,
    ...
    def forward(self, rnn_init_state=None, return_state=False, **inputs):
        ...
        rnn_inputs = TensorUtils.time_distributed(inputs,
                                                  self.nets["encoder"],
                                                  inputs_as_kwargs=True)
        assert rnn_inputs.ndim == 3  # [B, T, D]
        if self.per_step:
            return self.nets["rnn"].forward(inputs=rnn_inputs,
                                            rnn_init_state=rnn_init_state,
                                            return_state=return_state)

        # apply MLP + decoder to last RNN output
        outputs = self.nets["rnn"].forward(inputs=rnn_inputs,
                                           rnn_init_state=rnn_init_state,
                                           return_state=return_state)
        if return_state:
    ...
    def forward_step(self, rnn_state, **inputs):
        ...
        assert np.all([
            inputs[k].ndim - 1 == len(self.input_shapes[k])
...
class MIMO_Transformer(Module):
    ...
    def __init__(
        ...
        self.nets["embed_encoder"] = nn.Linear(transformer_input_dim,
                                               transformer_embed_dim)

        max_timestep = transformer_context_length

        if transformer_sinusoidal_embedding:
            self.nets["embed_timestep"] = PositionalEncoding(
                transformer_embed_dim)
            ...
            self.nets["embed_timestep"] = nn.Embedding(max_timestep,
                                                       transformer_embed_dim)
    ...
    def embed_timesteps(self, embeddings):
        ...
        ).unsqueeze(0).repeat(embeddings.shape[0], 1))
    ...
    def forward(self, **inputs):
        ...
        transformer_outputs[
            "transformer_encoder_outputs"] = transformer_encoder_outputs
