...
def get_demos_for_filter_key(hdf5_path, filter_key):
    ...
    demo_keys = [
        elem.decode("utf-8")
        for elem in np.array(f["mask/{}".format(filter_key)][:])
    ]
    f.close()
    return demo_keys
...
def get_env_metadata_from_dataset(dataset_path,
                                  set_env_specific_obs_processors=True):
    """
...
def get_shape_metadata_from_dataset(dataset_path,
                                    action_keys,
                                    all_obs_keys=None,
...
def get_motion_planner_metadata_from_dataset(dataset_path):
    ...
    if "mg_motion_plan_targets" in f["data"].attrs:
        cached_motion_planner_targets = json.loads(
            f["data"].attrs["mg_motion_plan_targets"])
    else:
    ...
    task_spec_obj, cached_motion_planner_targets_np = get_motion_planner_metadata_from_json_dicts(
        ...
        cached_motion_planner_targets_json=cached_motion_planner_targets,
    )
    return task_spec_obj, cached_motion_planner_targets_np


def get_motion_planner_metadata_from_json_dicts(
        task_spec_json=None, cached_motion_planner_targets_json=None):
    """
...
def get_intervention_segments(interventions):
    ...
    for i in range(1, len(expanded_ints)):
        if expanded_ints[i] and (not expanded_ints[i - 1]):
            ...
            start_inds.append(
                i - 1
            )  # record index in original array which is one less (since we added an element to the beg)
    ...
    assert np.all([
        np.sum(interventions[s:e]) == (e - s)
        for s, e in zip(start_inds, end_inds)
    ]), "window computation covers non-interventions"
    assert sum([
        np.sum(interventions[s:e]) for s, e in zip(start_inds, end_inds)
    ]) == np.sum(
        interventions), "window computation does not cover all interventions"
    ...
    for obs_cfg in obs_cfgs:
        ...
        if found_img:
            ...
            if "visual_feature_dimension" in old_encoder_cfg:
                rgb_encoder_cfg["core_kwargs"][
                    "feature_dimension"] = old_encoder_cfg[
                        "visual_feature_dimension"]

            if "visual_core" in old_encoder_cfg:
                rgb_encoder_cfg["core_kwargs"][
                    "backbone_class"] = old_encoder_cfg["visual_core"]

            for kwarg in ("pretrained", "input_coord_conv"):
                if "visual_core_kwargs" in old_encoder_cfg and kwarg in old_encoder_cfg[
                        "visual_core_kwargs"]:
                    rgb_encoder_cfg["core_kwargs"]["backbone_kwargs"][
                        kwarg] = old_encoder_cfg["visual_core_kwargs"][kwarg]
            ...
            for kwarg in ("num_kp", "learnable_temperature", "temperature",
                          "noise_std"):
                if "spatial_softmax_kwargs" in old_encoder_cfg and kwarg in old_encoder_cfg[
                        "spatial_softmax_kwargs"]:
                    rgb_encoder_cfg["core_kwargs"]["pool_kwargs"][
                        kwarg] = old_encoder_cfg["spatial_softmax_kwargs"][
                            kwarg]
...
def config_from_checkpoint(algo_name=None,
                           ckpt_path=None,
                           ckpt_dict=None,
                           verbose=False):
    """
...
def policy_from_checkpoint(device=None,
                           ckpt_path=None,
                           ckpt_dict=None,
                           verbose=False):
    """
    ...
    algo_name, _ = algo_name_from_checkpoint(ckpt_dict=ckpt_dict)
    config, _ = config_from_checkpoint(algo_name=algo_name,
                                       ckpt_dict=ckpt_dict,
                                       verbose=verbose)
    ...
    if obs_normalization_stats is not None:
        ...
        for m in obs_normalization_stats:
            for k in obs_normalization_stats[m]:
                obs_normalization_stats[m][k] = np.array(
                    obs_normalization_stats[m][k])

    # maybe restore action normalization stats
    action_normalization_stats = ckpt_dict.get("action_normalization_stats",
                                               None)
...
def env_from_checkpoint(ckpt_path=None,
                        ckpt_dict=None,
                        env_name=None,
                        env_class=None,
                        render=False,
    ...
    config, _ = config_from_checkpoint(algo_name=ckpt_dict["algo_name"],
                                       ckpt_dict=ckpt_dict,
                                       verbose=False)
    env = EnvUtils.wrap_env_from_config(
        env, config=config)  # apply environment wrapper, if applicable
    if verbose:
...
def download_url(url, download_dir, check_overwrite=True):
    ...
    assert url_is_alive(url), "@download_url got unreachable url: {}".format(
        url)
    time.sleep(0.5)
