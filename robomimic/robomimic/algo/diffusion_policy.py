...
lang_model = AutoModel.from_pretrained("distilbert-base-uncased",
...
class DiffusionPolicyUNet(PolicyAlgo):
    ...
    def _create_networks(self):
        ...
        if LangUtils.LANG_COND_ENABLED:
            obs_dim -= observation_group_shapes["obs"][
                LangUtils.LANG_OBS_KEY][0]
    ...
    def process_batch_for_training(self, batch):
        ...
        input_batch["obs"] = {
            k: batch["obs"][k][:, :To, :]
            for k in batch["obs"]  #if "raw" not in k
        }
        ...
        with torch.no_grad():
            if "raw_language" in batch["obs"].keys():
                raw_lang_strings = [
                    ...
                    for byte_string in batch["obs"]['raw_language']
                ]
                encoded_input = tokenizer(raw_lang_strings,
                                          padding=True,
                                          truncation=True,
                                          return_tensors='pt').to('cuda')
                outputs = lang_model(**encoded_input)
                encoded_lang = outputs.last_hidden_state.sum(
                    1).squeeze().unsqueeze(1).repeat(1, To, 1)
                input_batch["obs"][
                    "lang_fixed/language_distilbert"] = encoded_lang.type(
                        torch.float32)
        ...
        return TensorUtils.to_device(TensorUtils.to_float(input_batch),
                                     self.device)
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(DiffusionPolicyUNet,
                         self).train_on_batch(batch, epoch, validate=validate)
            ...
            noise = torch.randn([num_noise_samples] + list(actions.shape),
                                device=self.device)

            # sample a diffusion iteration for each data point
            timesteps = torch.randint(
                0,
                self.noise_scheduler.config.num_train_timesteps, (B, ),
    ...
    def get_action(self,
        ...
        if eval_mode:
            ...
            if goal_mode is not None:
                ...
                # Read in goal images
                goal_hand_camera_left_image = torch.FloatTensor((cv2.cvtColor(
                    cv2.imread(
                        os.path.join(root_path, f"{hand_camera_id}_left.png")),
                    cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                        2, 0, 1).unsqueeze(0).repeat([1, 1, 1, 1]).unsqueeze(0)
                goal_hand_camera_right_image = torch.FloatTensor((cv2.cvtColor(
                    cv2.imread(
                        os.path.join(root_path,
                                     f"{hand_camera_id}_right.png")),
                    cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                        2, 0, 1).unsqueeze(0).repeat([1, 1, 1, 1]).unsqueeze(0)
                goal_varied_camera_1_left_image = torch.FloatTensor(
                    (cv2.cvtColor(
                        cv2.imread(
                            os.path.join(root_path,
                                         f"{varied_camera_1_id}_left.png")),
                        cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                            2, 0, 1).unsqueeze(0).repeat([1, 1, 1,
                                                          1]).unsqueeze(0)
                goal_varied_camera_1_right_image = torch.FloatTensor(
                    (cv2.cvtColor(
                        cv2.imread(
                            os.path.join(root_path,
                                         f"{varied_camera_1_id}_right.png")),
                        cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                            2, 0, 1).unsqueeze(0).repeat([1, 1, 1,
                                                          1]).unsqueeze(0)
                goal_varied_camera_2_left_image = torch.FloatTensor(
                    (cv2.cvtColor(
                        cv2.imread(
                            os.path.join(root_path,
                                         f"{varied_camera_2_id}_left.png")),
                        cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                            2, 0, 1).unsqueeze(0).repeat([1, 1, 1,
                                                          1]).unsqueeze(0)
                goal_varied_camera_2_right_image = torch.FloatTensor(
                    (cv2.cvtColor(
                        cv2.imread(
                            os.path.join(root_path,
                                         f"{varied_camera_2_id}_right.png")),
                        cv2.COLOR_BGR2RGB) / 255.0)).cuda().permute(
                            2, 0, 1).unsqueeze(0).repeat([1, 1, 1,
                                                          1]).unsqueeze(0)

                obs_dict['camera/image/hand_camera_left_image'] = torch.cat(
                    [
                        obs_dict['camera/image/hand_camera_left_image'],
                        goal_hand_camera_left_image.repeat(1, To, 1, 1, 1)
                    ],
                    dim=2)
                obs_dict['camera/image/hand_camera_right_image'] = torch.cat(
                    [
                        obs_dict['camera/image/hand_camera_right_image'],
                        goal_hand_camera_right_image.repeat(1, To, 1, 1, 1)
                    ],
                    dim=2)
                obs_dict['camera/image/varied_camera_1_left_image'] = torch.cat(
                    [
                        obs_dict['camera/image/varied_camera_1_left_image'],
                        goal_varied_camera_1_left_image.repeat(1, To, 1, 1, 1)
                    ],
                    dim=2)
                obs_dict[
                    'camera/image/varied_camera_1_right_image'] = torch.cat(
                        [
                            obs_dict[
                                'camera/image/varied_camera_1_right_image'],
                            goal_varied_camera_1_right_image.repeat(
                                1, To, 1, 1, 1)
                        ],
                        dim=2)
                obs_dict['camera/image/varied_camera_2_left_image'] = torch.cat(
                    [
                        obs_dict['camera/image/varied_camera_2_left_image'],
                        goal_varied_camera_2_left_image.repeat(1, To, 1, 1, 1)
                    ],
                    dim=2)
                obs_dict[
                    'camera/image/varied_camera_2_right_image'] = torch.cat(
                        [
                            obs_dict[
                                'camera/image/varied_camera_2_right_image'],
                            goal_varied_camera_2_right_image.repeat(
                                1, To, 1, 1, 1)
                        ],
                        dim=2)
            # Note: currently assumes that you are never doing both goal and language conditioning
                ...
                with open(os.path.join(root_path, "lang_command.txt"),
                          'r') as file:
                    raw_lang = file.read()

                encoded_input = tokenizer(raw_lang,
                                          return_tensors='pt').to('cuda')
                outputs = lang_model(**encoded_input)
                encoded_lang = outputs.last_hidden_state.sum(
                    1).squeeze().unsqueeze(0).repeat(To, 1).unsqueeze(0)
                obs_dict["lang_fixed/language_distilbert"] = encoded_lang.type(
                    torch.float32)
    ...
    def _get_action_trajectory(self, obs_dict):
        ...
        obs_features = TensorUtils.time_distributed(
            {"obs": inputs["obs"]},
            nets['policy']['obs_encoder'].module,
            inputs_as_kwargs=True)
        assert obs_features.ndim == 3  # [B, T, D]
    ...
    def serialize(self):
        ...
        return {
            ...
            "ema":
            self.ema.averaged_model.state_dict()
